        ...
        case 'private_one_day':
            ...
            if($res = mysqli_query($connect, "INSERT INTO customer SET
                ...
                if($customer_id>0){
                    ...
                    $pay = initPay($customer_id, __FilterInt($_POST['price']));
                    $arrResult['pay']['status'] = $pay['status'];
                    $arrResult['pay']['data'] = $pay['data'];
                    $arrResult['pay']['LOG'] = $pay['log'];
                    $result = sendMail($mail);
                    $arrResult['log_session'] = $arrResult['pay']['data']['sessionId'];
                    setcookie("pay_sessionId", $arrResult['pay']['data']['sessionId'], time()+900, "/","pay.karelforum.ru",1);
